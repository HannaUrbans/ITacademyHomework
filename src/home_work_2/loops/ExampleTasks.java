...
public class ExampleTasks {
    public static void main(String[] args) {
        ...
        System.out.print(countedArray[0][0] + " чётных (");
        for (int i = 0; i < countedArray[0][0]; i++) {
            System.out.print(countedArray[2][i]);
            if (i < countedArray[0][0] - 1) {
                System.out.print(", ");
            }
        }
        System.out.print(") и ");
        System.out.print(countedArray[0][1] + " нечётных (");
        for (int i = 0; i < countedArray[0][1]; i++) {
            System.out.print(countedArray[1][i]);
            if (i < countedArray[0][1] - 1) {
                System.out.print(", ");
            }
        }
        System.out.println(")");
    ...
    public static int[][] countOddEvenDigits(int numToCount) {
        int tmp = Math.abs(numToCount);
        int numToCountLength = (int) (Math.floor(Math.log10(Math.abs(numToCount))) + 1);
        int oddCounter = 0;
        ...
        int[][] countedOddEven = {
                { oddCounter, evenCounter }, // Первый подмассив: количество нечетных и четных чисел
                oddArray, // Второй подмассив: массив нечетных чисел
                evenArray // Третий подмассив: массив четных чисел
        };
   ...
   // Вывести ряд чисел в диапазоне с шагом
    public static String lineOfNums (int minNum, int maxNum, int start){
        ...
        boolean first = true; // Флаг для первого числа
